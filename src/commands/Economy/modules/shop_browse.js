        ...
        try {
            ...
            const createShopEmbed = (page) => {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription('Используйте `/buy item_id:<id> quantity:<amount>` чтобы приобрести товар.');
                pageItems.forEach(item => {
                    embed.addFields({
                        name: `${item.name} (${item.id})`,
                        value: `🏷️ **Тип:** ${item.type}\n💚 **Цена:** $${item.price.toLocaleString()}\n${item.description}`,
                        inline: false,
                    });
                });
            ...
            collector.on('collect', async (buttonInteraction) => {
                if (buttonInteraction.user.id !== interaction.user.id) {
                    await buttonInteraction.reply({ content: '❌ Вы не можете использовать эти кнопки. Запустите "/shop browse", чтобы просмотреть свой собственный магазин.', flags: 64 });
                    return;
                }
